...
func (in *Interpreter) Run(contract *Contract, input []byte) (ret []byte, err error) {
	...
	fmt.Printf("Interpreter input len=%d  l=%d\n",len(input),len(contract.Input))
	if in.cfg.Debug {
		defer func() {
			if err != nil {
				if !logged {
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		if err := operation.validateStack(stack); err != nil {
			fmt.Errorf("invalid stack ")
			return nil, err
		}
		//fmt.Printf("before operation.execute 2 \n")
		// If the operation is valid, enforce and write restrictions
		if err := in.enforceRestrictions(op, operation, stack); err != nil {
			fmt.Errorf("invalid enforceRestrictions ")
			return nil, err
		}
		//fmt.Printf("before operation.execute 3 \n")
		...
		if operation.memorySize != nil {
			...
			if overflow {
				fmt.Errorf("invalid errGasUintOverflow ")
				fmt.Printf("overflow memsize=%d\n",memSize)
				return nil, errGasUintOverflow
			}
			//fmt.Printf("before operation.execute 31 \n")
		...
		//fmt.Printf("before operation.execute 4\n")
		if memorySize > 0 {
		...
		//fmt.Printf("before operation.execute \n")
